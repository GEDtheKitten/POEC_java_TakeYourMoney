...
BEGIN
	SELECT * 
	FROM Clients
    WHERE client_nom = client_nom AND client_prenom = client_prenom;
END //
...
BEGIN
	...
	FROM Produits AS p
    JOIN Prix AS px on p.produit_ID = px.produit_ID
    WHERE p.produit_designation = p.produit_designation;
END //
...
CREATE PROCEDURE P_modifier_produit (IN produit_designation VARCHAR(50), IN produit_desactive BOOLEAN, 
									IN prix_date DATE, prix_HT DECIMAL(15,2))
BEGIN
    UPDATE Produits
	SET produit_designation = produit_designation, produit_desactive = produit_desactive
    WHERE produit_ID = produit_ID;
    UPDATE Prix
	SET prix_date = prix_date, prix_HT = prix_HT
    WHERE produit_ID = produit_ID;
END //


-- Procedure - modifier le prix d'un produit
CREATE PROCEDURE P_modifier_prix_produit (IN produit_designation VARCHAR(50), IN produit_desactive BOOLEAN, 
									IN prix_date DATE, prix_HT DECIMAL(15,2))
BEGIN
    UPDATE Produits
	SET produit_designation = produit_designation, produit_desactive = produit_desactive
